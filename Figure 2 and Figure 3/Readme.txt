data_flow folder is composed by:
 ...
 - MYKSQI: original code provided in https://github.com/zduanmu/ksqi, with minimum modification in order to be runned in my environment.

 - data: contains data processed through all the pipeline, results and plots 
 ...
 - run_models_2.py: it takes as input the generated files from the above script and then it run the already trained models (on WaterlooII and WaterlooI datasets) of https://ece.uwaterloo.ca/~zduanmu/waterloosqoe4/ with 
 ...
 - plot_cdf_4.py: file used to plot the Fig 5&6 of the paper


How to replicate the plots:
in all the code an absolute path to data is used, so it is needed a modification by changing the paths variables in accordance to your environment.
